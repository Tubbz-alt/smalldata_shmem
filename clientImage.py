...
from smalldata_tools import l3tDetector
...
def runclient(args):
    ...
    smlDets = []
    smlDets.append(L3tDetectors()
    #defaultDets = defaultDetectors(hutch)
    #defaultDets.append(ebeamDetector())

    dets={}
    detNames=[]
    setupDict=yaml.load(open('detectorImage.yml','r'))
    for plot in setupDict.keys():
        detname = setupDict[plot][detector]
        if detname not in dets.keys():
            dets[detname]=DetObject(detNames[-1], common_mode=setupDict[plot][common_mode])
        if setupDict[plot][dataType]=='photon':
            dets[detname].addPhoton(setupDict[plot][photon_ADU])
